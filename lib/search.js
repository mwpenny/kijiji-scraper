...
function getPageResults(params, pageNum, firstResultPagePath) {
    ...
    // Search Kijiji
    return fetch(KIJIJI_BASE_URL + pagePath)
        .then(res => res.text())
        .then(body => {
            const results = parseResultsHTML(body);

            /* TODO: Throw this error again once a better way of detecting the last result
               page is implemented. Kijiji stores this in some JS in the result page as the
               `isLastPage` flag
...
async function getSearchResults(params, minResults) {
    ...
    while (true) {
        const { pageResults } = await getPageResults(params, pageNum, firstResultPagePath);
        results.push(...pageResults);

        /* TODO: Kijiji stores a flag in the page indicating whether or not the current page
           is the last one (`isLastPage`). That should be used instead of checking for 0
           results or a hard-coded maximum */
