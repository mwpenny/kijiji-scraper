...
function parseResultsHTML(html) {
    ...
    filteredAdElements.each(function(i, item) {
        try {
            ...
            let info = {
                "title": $(item).find(".description > .title").text().trim(),
                "image": $(item).find(".image img").attr("src").replace(IMG_REGEX, "/$_57.JPG"),
                "date": dateFromRelativeDateString($(item).find(".posted").text().trim()),
    ...
    return { ads: adResults, excludedAdCount: allAdElements.length - filteredAdElements.length };
}
...
function getSearchResults(params, minResults, pageNum=1, results=[], firstResultPagePath="") {
    return getPageResults(params, pageNum, firstResultPagePath).then(function(ret) {
        results.push(...ret.pageResults.ads);
        if (results.length >= minResults ||
            (ret.pageResults.ads.length + ret.pageResults.excludedAdCount) < MAX_RESULTS_PER_PAGE ||
