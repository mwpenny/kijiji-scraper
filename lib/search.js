...
function initFirstResultPagePath(params, firstResultPagePath="") {
    return new Promise(function(resolve, reject) {
        ...
        request({"url": KIJIJI_SEARCH_URL, "qs": params, followRedirect: false}, function(err, res) {
            /* followRedirect is passed as as false which will allow the redirect to
               be captured in most environments. However, if running in a web browser,
               the redirect will be transparently followed anyway - regardless of
               followRedirect's value */
            if (err)
                reject(err);
            else if (res.statusCode == 301)
                resolve(res.caseless.get("location"));
            else if (res.statusCode == 200)
                resolve(res.request.uri.path);
            else
                reject(new Error("Kijiji failed to redirect to search results"));
        });
    });
}
...
function getPageResults(params, pageNum, firstResultPagePath="") {
    ...
    return new Promise(function(resolve, reject) {
        initFirstResultPagePath(params, firstResultPagePath).then(function(location) {
            // Specify page number. It must be the last path component of the URL
            let pageLocation = location.replace(LOCATION_REGEX, `$1/page-${pageNum}$2`);

            // Search Kijiji
            request(KIJIJI_BASE_URL + pageLocation, function(err, res) {
                if (err)
                    return reject(err);
                ...
                resolve({pageResults: results, firstResultPagePath: location});
            });
        });
    });
}
